...
## Sprint 3.4 - Public Repositioning To Hakan Leonardo

Status: completed locally.

Goal: reposition this repository as Hakan Leonardo's public business/services website while preserving the existing technical architecture, deterministic AI discovery assistant, and lead-capture path.

Result:

- Public-facing brand/copy moved from Leonardo Tales to Hakan Leonardo where appropriate.
- Homepage and metadata now describe practical AI systems: AI customer representative, AI sales/proposal assistant, and AI operations automation.
- Chat assistant opening, casual reply, form handoff, success copy, API success response, and Telegram lead notification title were rebranded.
- README and `public/llms.txt` now state that this repository is not Leonardo Tales Core, Dev, Science, agent OS, or a software factory.
- Core/Dev/Science and agent OS implementation remains intentionally outside this repository.
- Existing UI style, routes, lead API, Supabase persistence path, Telegram notification path, and environment/deployment settings were preserved.

Remaining:

- Historical sprint log references to Leonardo Tales remain as project history unless they become public-facing confusion.
- `source: "leonardo_tales_ai_representative"` remains an internal lead contract and should only be changed with a schema/data migration decision.
- Production domain/default site URL still needs a later deployment/domain decision.

## Sprint 3.5 - Vision Rename, Single Assistant Surface & Public Site Completion

Status: completed locally.

Goal: complete the Hakan Leonardo public-site alignment with a single active assistant surface, Vizyon route/nav, practical service flow, and deterministic safety guardrails.

Result:

- Public nav now shows `/vizyon` instead of `/manifesto`.
- Added `/vizyon` and `/en/vision` routes.
- Kept `/manifesto` and `/en/manifesto` as redirects so old links do not break.
- Replaced the long manifesto content with Hakan Leonardo Vizyonu copy and principles.
- Moved `CoreAiChat` into the hero as the single active assistant surface and removed the lower duplicate chat section.
- Header, hero, and final CTA all point to the same `#assistant` surface.
- Added a compact `Nasıl Çalışır` / `How It Works` process section.
- Added deterministic harmful-intent refusal and oversized-product MVP framing to the chat flow and evals.
- Kept OpenAI/LLM, deploy, env, secrets, database migration, and Leonardo Tales Core/Dev/Science scope untouched.

Remaining:

- CSS class names with `manifesto-*` may remain as internal styling names only; public visible copy is Vizyon.
- Exact production domain/default `NEXT_PUBLIC_SITE_URL` decision remains postponed.

## Sprint 3.6 - Hero Chat Repair, Process Polish & Vision Simplification

Status: completed locally.

Goal: repair the hero assistant surface and refine the public Hakan Leonardo service-site flow without adding product scope, deploy work, OpenAI/LLM, integrations, or broad redesign.

Result:

- Removed the visible `ACTIVE_PROTOCOLS` sidebar from the single public `CoreAiChat` surface.
- Replaced technical chat title copy with `Yapay Zekâ İş Temsilcisi` and `HAKAN_LEONARDO / ONLINE`.
- Kept the deterministic chat behavior, safety guardrails, pricing guardrail, MVP framing, and form timing intact.
- Removed `/vizyon` from the public navbar; `/vizyon` and `/en/vision` routes remain available.
- Changed the hero secondary CTA to point to `#systems`.
- Simplified the homepage vision section to a focused Hakan Leonardo Vizyonu paragraph and quote.
- Removed the Soyutlama / Zekâ / Kontrol / Etki cards from the homepage vision section.
- Polished `Nasıl Çalışır` into four premium process cards using the existing dark terminal/system visual language.
- Verified 390-requested, 768, and 1440 viewport QA through Firefox/geckodriver with no horizontal overflow and one active chat surface.
- `npm run eval:agent`, `npm run eval:leads`, `npm run lint`, and `npm run build` passed.

Remaining:

- Firefox headless still reports a minimum mobile inner width around 500px when 390px is requested; real-device 390px QA remains useful.
- Browser-plugin console capture is still unavailable in this local setup.
