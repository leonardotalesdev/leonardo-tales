...
- Briefly explains how Hakan Leonardo can help the visitor's business type when enough context exists.
...
Sprint 3.4 should refine this further: the opening should feel more distinctive to Hakan Leonardo, intelligent, warm, slightly witty, and professional. It should invite the visitor to write naturally, then guide them toward:
...
1. Greeting:
   - "Merhaba. Ben Hakan Leonardo’nun yapay zekâ keşif asistanıyım. İşinizi, projenizi veya kurmak istediğiniz yapay zekâ sistemini kısaca anlatın; size doğru başlangıç noktasını netleştireyim."
...
If the user is casual or just browsing, the assistant should not push the form. It should briefly explain what Hakan Leonardo usually builds and ask what the user does.
...
Sprint 3.0 added `docs/13_AGENT_CONTROL_PRINCIPLES.md` as the long-term reference for designing, limiting, monitoring, and controlling business agents.

As of the repositioning sprint, this repository should apply those principles only to the public Hakan Leonardo service-site MVP. It should not begin Leonardo Tales Core, Dev, Science, agent OS, or software factory implementation.
